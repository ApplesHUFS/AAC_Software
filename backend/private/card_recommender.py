...
class CardRecommender:
    ...
    def __init__(self, clustering_results_path: str, config: Dict[str, Any]):
        ...
        if Path(clustering_results_path).exists():
            ...
            self.clustered_files = {
                int(k): v for k, v in cluster_data["clustered_files"].items()
            }
            self.all_cards = cluster_data.get("filenames", [])
            self.n_clusters = max(cluster_data["cluster_labels"]) + 1
        else:
            raise FileNotFoundError(
                f"클러스터링 결과 파일이 필요합니다: {clustering_results_path}"
            )
    ...
    def get_card_selection_interface(
        self, persona: Dict[str, Any], context: Dict[str, Any], context_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # 상황 기반 카드 추천
            context_cards = self._select_context_based_cards(
                context, int(total_cards * context_ratio)
            )

            # 페르소나 기반 카드 추천
            persona_cards = self._select_persona_based_cards(
                persona, int(total_cards * (1 - context_ratio))
            )
            ...
            interface_data = {
                ...
                "total_pages": self.recommendation_history[context_id][-1][
                    "page_number"
                ],
            }
            ...
            activity_info = (
                f" ({context.get('current_activity')} 활동 고려)"
    ...
    def _select_context_based_cards(
    ...
    def _select_persona_based_cards(
        self, persona: Dict[str, Any], target_count: int
    ) -> List[str]:
    ...
    def _find_similar_clusters(
        self, keywords: List[str], similarity_threshold: float, max_clusters: int
        ...
        for cluster_id, cluster_tags in self.cluster_tags.items():
            ...
            for keyword in keywords:
                for tag in cluster_tags:
                    try:
                        similarity = self.cluster_calculator.compute_topic_similarity(
                            keyword, tag
                        )
        ...
        sorted_clusters = sorted(
            cluster_similarities.items(), key=lambda x: x[1], reverse=True
        )
        return sorted_clusters[:max_clusters]

    def _select_cards_from_clusters(
        self, cluster_scores: List[Tuple[int, float]], target_count: int
    ) -> List[str]:
        ...
        for cluster_id, score in cluster_scores:
            ...
            # 클러스터에서 랜덤 선택
            available_cards = [
                card for card in cluster_cards if card not in selected_cards
            ]
            if available_cards:
    ...
    def _combine_card_selections(
        self, context_cards: List[str], persona_cards: List[str], total_count: int
    ) -> List[str]:
        ...
        if remaining_count > 0:
            additional_cards = self._select_random_cards(
                list(used_cards), remaining_count
            )
            combined_cards.extend(additional_cards)

        return combined_cards[:total_count]

    def _select_from_preferred_clusters(
        self, preferred_clusters: List[int], num_cards: int
    ) -> List[str]:
    ...
    def _select_random_cards(
        self, exclude_cards: List[str], num_cards: int
    ) -> List[str]:
        """전체 카드에서 랜덤으로 선택합니다.
    ...
    def validate_card_selection(
        ...
        invalid_cards = [
            card for card in selected_cards if card not in available_options
        ]
        if invalid_cards:
            return {
                "status": "error",
    ...
    def _add_to_recommendation_history(
        self, context_id: str, cards: List[str]
    ) -> Dict[str, Any]:
        """카드 추천 히스토리에 추가합니다.
    ...
    def get_recommendation_history_page(
