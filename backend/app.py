"""
AAC Interpreter Backend
Copyright (c) 2025 APPLES
...
def handle_error(error):
    ...
    traceback.print_exc()
    return api_response(
        success=False, error="서버 내부 오류가 발생했습니다", status_code=500
    )
...
def serve_image(filename):
    ...
    try:
        images_folder = SERVICE_CONFIG["images_folder"]
...
def register():
    ...
    try:
        ...
        if not data:
            return api_response(
                success=False, error="유효한 JSON 데이터가 필요합니다", status_code=400
            )
        ...
        persona_data = {
            ...
            "communication_characteristics": data.get(
                "communicationCharacteristics", ""
            ),
        ...
        if result["status"] == "success":
            ...
            return api_response(success=False, error=result["message"], status_code=400)
...
def login():
    ...
    try:
        ...
        if result["authenticated"]:
            ...
            return api_response(success=False, error=result["message"], status_code=401)
        ...
        if result["status"] == "success":
            ...
            return api_response(
                data={
                    ...
                    "communicationCharacteristics": user_data.get(
                        "communication_characteristics"
                    ),
                    "interestingTopics": user_data.get("interesting_topics", []),
                    "preferredCategoryTypes": user_data.get(
                        "preferred_category_types", []
                    ),
            ...
            return api_response(success=False, error=result["message"], status_code=404)
...
def create_context():
    ...
    try:
        ...
        if not user_id:
            return api_response(
                success=False, error="userId가 필요합니다", status_code=400
            )
        if not place:
            return api_response(
                success=False, error="place는 필수 입력사항입니다", status_code=400
            )
        if not interaction_partner:
            return api_response(
                success=False,
                error="interactionPartner는 필수 입력사항입니다",
                status_code=400,
            )
...
def recommend_cards():
    ...
    try:
        ...
        if not user_id or not context_id:
            return api_response(
                success=False, error="userId와 contextId가 필요합니다", status_code=400
            )
        ...
        # 카드 추천 시스템 확인
        if (
            not hasattr(aac_service, "card_recommender")
            or aac_service.card_recommender is None
        ):
            return api_response(
                success=False,
        ...
        result = aac_service.get_card_selection_interface(
            user_id, context_data, context_id
        )
        ...
        if result["status"] == "success":
            ...
            return api_response(
                data={
                    ...
                    "contextInfo": {
                        ...
                        "interactionPartner": interface_data.get(
                            "context_info", {}
                        ).get("interaction_partner"),
                        "currentActivity": interface_data.get("context_info", {}).get(
                            "current_activity"
                        ),
                    },
                    "selectionRules": {
                        "minCards": interface_data.get("selection_rules", {}).get(
                            "min_cards", 1
                        ),
                        "maxCards": interface_data.get("selection_rules", {}).get(
                            "max_cards", 4
                        ),
            ...
            return api_response(success=False, error=result["message"], status_code=500)
...
def get_card_history_page(context_id, page_number):
    """특정 페이지의 카드 히스토리"""
    try:
        result = aac_service.get_card_recommendation_history_page(
...
def validate_cards():
    ...
    try:
        ...
        if not selected_cards:
            return api_response(
                success=False, error="선택된 카드가 없습니다", status_code=400
            )

        result = aac_service.validate_card_selection(selected_cards, available_options)

        return api_response(
            data={"valid": result["valid"], "selectedCount": len(selected_cards)},
            message=result["message"],
...
def interpret_cards():
    ...
    try:
        ...
        if context_id:
            ...
            if context_result["status"] == "success":
                context = {
                    "time": context_result["context"]["time"],
                    "place": context_result["context"]["place"],
                    "interaction_partner": context_result["context"][
                        "interaction_partner"
                    ],
                    "current_activity": context_result["context"]["current_activity"],
                }
        ...
        # 카드 해석기 확인
        if (
            not hasattr(aac_service, "card_interpreter")
            or aac_service.card_interpreter is None
        ):
            return api_response(
                success=False,
...
def submit_feedback():
    ...
    try:
        ...
        if not confirmation_id:
            return api_response(
                success=False, error="confirmationId가 필요합니다", status_code=400
            )
        ...
        if result["status"] == "success":
            return api_response(
                data={"feedbackResult": result["feedback_result"]},
                message=result["message"],
            )
        else:
            return api_response(success=False, error=result["message"], status_code=400)
