  ...
  async getHistorySummary(contextId) {
    try {
      ...
      if (response.success && response.data) {
        ...
        throw new Error(
          response.error || "히스토리 정보를 불러올 수 없습니다."
        );
      }
    } catch (error) {
      throw error;
  ...
  async getHistoryPage(contextId, pageNumber) {
    try {
      ...
      const response = await api.get(
        `/api/cards/history/${contextId.trim()}/page/${pageNumber}`
      );
      ...
      if (response.success && response.data?.cards) {
        ...
        throw new Error(
          response.error || "히스토리 페이지를 불러올 수 없습니다."
        );
      }
    } catch (error) {
      throw error;
      ...
      .map((card, index) => {
        if (typeof card === "string") {
          ...
          return {
            ...
            name:
              card.name ||
              card.filename?.replace(".png", "").replace(/_/g, " ") ||
              `Card ${index + 1}`,
