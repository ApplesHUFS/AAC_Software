...
const CardItem = ({ card, isSelected, onSelect, disabled = false }) => {
  ...
  return (
    <div
      className={`card-item communicator-card ${isSelected ? "selected" : ""} ${
        disabled ? "disabled" : ""
      } ${imageError ? "error" : ""}`}
      onClick={handleClick}
    ...
    <div className="selected-cards-display communicator-sidebar">
      ...
      <div className="selected-cards-list">
        {selectedCards.map((card, index) => (
          <div
            key={`selected-${card.filename}-${index}`}
            className="selected-card-item communicator-selected-item"
          >
      ...
      <div className="selection-summary communicator-summary">
        <p>
          {maxCards - selectedCards.length > 0 ? (
            <>
              <strong>{maxCards - selectedCards.length}개</strong> 더 선택할 수
              있어요!
            </>
          ) : (
            <>모든 카드를 선택했어요!</>
