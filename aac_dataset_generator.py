import json
import os
import base64
from pathlib import Path
from typing import List, Dict, Any
from openai import OpenAI
from dotenv import load_dotenv
from tqdm import tqdm
import time

load_dotenv()

class AACDatasetGenerator:
    def __init__(self, dataset_path: str, images_folder: str, output_path: str = None):
        self.dataset_path = dataset_path
        self.images_folder = images_folder
        self.output_path = output_path or dataset_path.replace('.json', '_completed.json')
        
        self.client = OpenAI()
        
        with open(dataset_path, 'r', encoding='utf-8') as f:
            self.dataset = json.load(f)
            
        print(f"데이터셋 로드 완료: {len(self.dataset)}개 샘플")
        
    def encode_image(self, image_path: str) -> str:
        with open(image_path, "rb") as image_file:
            return base64.b64encode(image_file.read()).decode('utf-8')
    
    def prepare_image_content(self, card_combinations: List[str]) -> List[Dict]:
        content = []
        
        content.append({
            "type": "text",
            "text": "다음 AAC 카드들을 순서대로 보고 해석해주세요:"
        })
        
        for i, card_filename in enumerate(card_combinations, 1):
            image_path = os.path.join(self.images_folder, card_filename)
            
            if os.path.exists(image_path):
                base64_image = self.encode_image(image_path)
                
                content.append({
                    "type": "text",
                    "text": f"\n카드 {i}:"
                })
                
                content.append({
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{base64_image}",
                        "detail": "low"  # 비용 절감을 위해 low 사용
                    }
                })
            else:
                print(f"경고: 이미지 파일을 찾을 수 없음 - {image_path}")
                raise FileNotFoundError(f"필수 이미지 파일 없음: {image_path}")
                
        return content
    
    def generate_context(self, persona: Dict, card_combination: List[str]) -> Dict:
        
        system_prompt = """당신은 AAC(보완대체의사소통) 전문가입니다. 
제공된 AAC 카드 이미지들을 보고, 페르소나 정보와 함께 이 상황이 일어날 수 있는 현실적인 맥락(context)을 생성해주세요.

이미지를 직접 보고 판단하여, 자연스럽게 연결되는 일상적인 상황을 만들어주세요."""
        
        image_content = self.prepare_image_content(card_combination)
        
        user_content = [
            {
                "type": "text",
                "text": f"""페르소나:
- 나이: {persona['age']}
- 성별: {persona['gender']}
- 장애 유형: {persona['disability_type']}
- 의사소통 특성: {persona['communication_characteristics']}

위 페르소나가 제공된 AAC 카드 이미지들을 사용하는 상황을 상상해주세요."""
            }
        ]
        
        user_content.extend(image_content)
        
        user_content.append({
            "type": "text",
            "text": """
위 이미지들을 보고 다음 형식의 JSON으로 context를 생성해주세요:
{
    "time": "오전/오후 X시 형식 또는 아침/점심/저녁 등",
    "place": "구체적인 장소 (예: 학교 교실, 집 거실, 치료실 등)",
    "interaction_partner": "대화 상대 (예: 엄마, 선생님, 친구, 치료사 등)",
    "current_activity": "현재 하고 있는 활동이나 상황"
}

실제 일어날 법한 자연스러운 상황으로 만들어주세요."""
        })
        
        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",  # vision 지원 모델
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_content}
                ],
                response_format={"type": "json_object"},
                temperature=0.8,
                max_tokens=300
            )
            
            context = json.loads(response.choices[0].message.content)
            return context
            
        except Exception as e:
            print(f"Context 생성 중 오류: {e}")
            return None

    def generate_interpretations(self, persona: Dict, context: Dict, 
                               card_combination: List[str]) -> List[str]:
        system_prompt = """당신은 AAC(보완대체의사소통) 해석 전문가입니다.
제공된 AAC 카드 이미지들을 직접 보고, 주어진 페르소나와 상황(context)을 고려하여 3가지 해석을 생성해주세요.

해석 원칙:
1. 첫 번째 해석: 이미지 순서와 내용을 보고 가장 가능성이 높고 직관적인 해석
2. 두 번째 해석: 같은 이미지들을 다른 관점에서 해석
3. 세 번째 해석: 창의적이면서도 맥락상 가능한 해석

각 해석은 자연스러운 한국어 문장으로 작성하되, 사용자의 의도를 명확히 전달해야 합니다.
이미지의 시각적 요소와 순서를 중요하게 고려하세요."""
        
        image_content = self.prepare_image_content(card_combination)
        
        user_content = [
            {
                "type": "text",
                "text": f"""페르소나:
- 나이: {persona['age']}
- 성별: {persona['gender']}
- 장애 유형: {persona['disability_type']}
- 의사소통 특성: {persona['communication_characteristics']}

상황(Context):
- 시간: {context['time']}
- 장소: {context['place']}
- 대화 상대: {context['interaction_partner']}
- 현재 활동: {context['current_activity']}

아래 AAC 카드 이미지들을 순서대로 보고 해석해주세요:"""
            }
        ]
        
        user_content.extend(image_content)
        
        user_content.append({
            "type": "text",
            "text": """
위 이미지들을 보고 이 사람이 전달하고자 하는 메시지를 3가지로 해석해주세요.
이미지의 시각적 내용과 순서를 반드시 고려하세요.

JSON 형식으로 응답해주세요:
{
    "interpretations": [
        "첫 번째 해석 (가장 가능성 높음)",
        "두 번째 해석 (다른 관점)",
        "세 번째 해석 (창의적)"
    ]
}"""
        })
        
        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",  # vision 지원 모델
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_content}
                ],
                response_format={"type": "json_object"},
                temperature=0.9,
                max_tokens=400
            )
            
            result = json.loads(response.choices[0].message.content)
            return result.get("interpretations", [])
            
        except Exception as e:
            print(f"해석 생성 중 오류: {e}")
            return None
    
    def process_dataset(self, start_idx: int = 0, end_idx: int = None, 
                       save_interval: int = 10):
        if end_idx is None:
            end_idx = len(self.dataset)
            
        print(f"\n처리 시작: {start_idx} ~ {end_idx-1} ({end_idx-start_idx}개)")
        print("주의: 모든 처리는 실제 이미지를 기반으로 합니다.\n")
        
        processed_count = 0
        error_count = 0
        
        for idx in tqdm(range(start_idx, end_idx), desc="데이터 처리"):
            item = self.dataset[idx]
            
            if item['input']['context'].get('time') and item.get('output'):
                continue
            
            try:
                persona = item['input']['persona']
                card_combination = item['input']['AAC_card_combination']
                
                for card in card_combination:
                    image_path = os.path.join(self.images_folder, card)
                    if not os.path.exists(image_path):
                        raise FileNotFoundError(f"이미지 없음: {card}")
                
                # 1. Context 생성 (이미지 기반)
                context = self.generate_context(persona, card_combination)
                item['input']['context'] = context
                
                # 2. 해석 생성 (이미지 기반)
                interpretations = self.generate_interpretations(
                    persona, context, card_combination
                )
                item['output'] = interpretations
                
                processed_count += 1
                
                if (processed_count) % save_interval == 0:
                    self.save_dataset(f"{self.output_path}.tmp")
                    print(f"\n중간 저장 완료: {processed_count}개 처리, {error_count}개 오류")
                
            except Exception as e:
                print(f"\n샘플 {idx} 처리 중 오류: {e}")
                error_count += 1
                item['input']['context'] = {
                    "time": "오류",
                    "place": "오류",
                    "interaction_partner": "오류",
                    "current_activity": "오류"
                }
                item['output'] = ["처리 오류"]
            
            time.sleep(1)
        
        self.save_dataset(self.output_path)
        print(f"\n처리 완료!")
        print(f"성공: {processed_count}개, 오류: {error_count}개")
        print(f"결과 저장: {self.output_path}")
    
    def save_dataset(self, path: str = None):
        save_path = path or self.output_path
        with open(save_path, 'w', encoding='utf-8') as f:
            json.dump(self.dataset, f, ensure_ascii=False, indent=2)
    
    def print_sample(self, idx: int = 0):
        if idx < len(self.dataset):
            item = self.dataset[idx]
            print(f"\n=== 샘플 {idx} ===")
            print(f"페르소나: {item['input']['persona']['age']} {item['input']['persona']['gender']}")
            print(f"장애 유형: {item['input']['persona']['disability_type']}")
            print(f"\n카드 조합 (이미지):")
            for card in item['input']['AAC_card_combination']:
                print(f"  - {card}")
                image_path = os.path.join(self.images_folder, card)
                if os.path.exists(image_path):
                    print(f"    ✓ 이미지 확인됨")
                else:
                    print(f"    ✗ 이미지 없음!")
            print(f"\nContext:")
            print(json.dumps(item['input']['context'], ensure_ascii=False, indent=2))
            print(f"\n해석:")
            if item.get('output'):
                for i, interp in enumerate(item['output'], 1):
                    print(f"  {i}. {interp}")


if __name__ == "__main__":
    generator = AACDatasetGenerator(
        dataset_path="data/aac_dataset_with_cards.json",
        images_folder="data/images",
        output_path="data/aac_dataset_completed.json"
    )
    
    print("\n=== 처리 전 상태 확인 ===")
    generator.print_sample(0)
    
    # 데이터셋 처리 (처음 5개만 테스트)
    print("\n=== 이미지 기반 처리 시작 ===")
    generator.process_dataset(start_idx=0, end_idx=5, save_interval=2)
    
    # 처리 후 샘플 확인
    print("\n=== 처리 후 결과 ===")
    generator.print_sample(0)
    
    # 전체 처리하려면:
    # generator.process_dataset()
    
    # 특정 구간 이어서 처리:
    # generator.process_dataset(start_idx=5, end_idx=100)